...
export class TransactionsService {
  ...
  async create(
    ...
    try {
      ...
      if (user.role === UserRole.STAFF) {
        baseQuery.andWhere("inv.warehouseId = :warehouseId", {
          warehouseId: user.warehouseId,
        });
      } else {
        baseQuery.andWhere("warehouse.companyId = :companyId", {
          companyId: user.companyId,
        });
      }

      const inventory = await baseQuery.getOne();
      ...
      this.logger.error(
        `Transaction creation failed, rolling back`,
  ...
  async findAll(
    ...
    if (user.role === UserRole.STAFF) {
      ...
      qb.where("inv.warehouseId = :assignedWh", {
    ...
    if (warehouseInventoryId) {
      qb.andWhere("tx.warehouseInventoryId = :warehouseInventoryId", {
        warehouseInventoryId,
      });
    }
    if (type) {
  ...
  async findOne(
    ...
    if (user.role === UserRole.STAFF) {
      qb.andWhere("inv.warehouseId = :assignedWh", {
        assignedWh: user.warehouseId,
      });
    } else {
      qb.andWhere("warehouse.companyId = :companyId", {
        companyId: user.companyId,
      });
    }
