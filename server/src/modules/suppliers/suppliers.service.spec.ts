...
describe("SuppliersService", () => {
  ...
  describe("findAllByCompany", () => {
    ...
    it("applies ILIKE search filter across companyName, contactPerson, phone, and email when search is supplied", async () => {
      const query: QuerySupplierDto = {
        search: "acme",
        page: 1,
        limit: 10,
    ...
    it("applies an exact-match companyName filter when the dropdown filter is supplied (distinct from fuzzy search)", async () => {
      const query: QuerySupplierDto = {
        companyName: "Acme Corp",
        page: 1,
        limit: 10,
    ...
    it("calculates correct pagination skip/take and returns standard paginated envelope", async () => {
      const query: QuerySupplierDto = { page: 2, limit: 5 } as QuerySupplierDto;
    ...
    it("returns the SAME paginated envelope shape (not a raw array) when all=true bypasses pagination", async () => {
      const query: QuerySupplierDto = {
        page: 1,
        limit: 10,
        all: true,
      ...
      // Must be the full envelope, NOT the raw array — this exact mismatch
      // caused a real frontend crash (warehouses.map is not a function)
      // when a prior version of this pattern returned a bare array here.
      expect(result).toEqual({
        data: mockItems,
        page: 1,
  ...
  describe("update", () => {
    it("updates and saves supplier when ownership check succeeds", async () => {
      ...
      mockRepo.save.mockImplementation((entity: Supplier) =>
  ...
  describe("remove", () => {
    ...
    it("throws NotFoundException if target supplier does not exist for company", async () => {
      ...
      await expect(
        service.remove(mockCompanyId, "sup-foreign"),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
