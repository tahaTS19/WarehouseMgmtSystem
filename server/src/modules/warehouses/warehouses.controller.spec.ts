...
describe('WarehousesController', () => {
  ...
  it('delegates create to service using requester companyId from JWT', async () => {
    ...
    const dto: CreateWarehouseDto = { name: 'Central Warehouse', location: 'Lahore' };
    mockWarehousesService.create.mockResolvedValue({ id: 'wh-1', companyId: 'company-1', ...dto });
  ...
  it('delegates findAll to service with companyId', async () => {
    const mockReq = { user: { companyId: 'company-1' } } as any;
    mockWarehousesService.findAllByCompany.mockResolvedValue([{ id: 'wh-1', name: 'Main' }]);
  ...
  it('delegates findOne to service with companyId and warehouse id', async () => {
    const mockReq = { user: { companyId: 'company-1' } } as any;
    mockWarehousesService.findOne.mockResolvedValue({ id: 'wh-1', name: 'Main' });
